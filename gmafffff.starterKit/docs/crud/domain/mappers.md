# Автоматическое преобразование сущность ⟷ DTO

## Общие сведения

Пространства имен: `gmafffff.starterKit.Mappers`.

Для единообразного преобразования сущностей в DTO и обратно объявлена группа интерфейсов:

* `IEntityMapperBackward` — DTO → сущность;
* `IEntityMapperForward` — сущность → DTO;
* `IEntityMapperDuplex` — сущность ⇆ DTO;
* `IEntityMapperForwardExpression` — Expression для проекции сущность → DTO;
* `IEntityMapper` — интерфейс-маркер.

## Использование

Реализация интерфейсов предполагается с помощью кодогенерации от Mapster.
Основная опасность использования авто преобразователей типов — скрытие факта неполной трансформации объекта.
Для навязывания политики предварительной проверки конфигурации необходимо регистрировать
преобразователи с помощью метода расширения `IServiceCollection.AddEntityMappersAndConfig`.

Вместо наследования от интерфейса `Mapster.IRegister` конфигурацию преобразователя
можно размещать в классе, производном от `StandardMapsterConfig`.
Он поможет единообразно формировать статические методы расширения.

Регистрация преобразователей в контейнере DI и регистрация конфигураций преобразователей осуществляется
вызовом `AddEntityMappers` — метода расширения `IServiceCollection`.

## DI

Регистрация в контейнере DI осуществляется во время вызова
метода расширения `IServiceCollection.AddEntityMappersAndConfig`.
